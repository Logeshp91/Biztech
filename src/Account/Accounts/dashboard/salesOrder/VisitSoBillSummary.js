...
const VisitSoBillSummary = ({ route }) => {
  // Map products to expected fields
  const { products: rawProducts = [], form = {}, taxDetails = [] } = route.params;

  const products = rawProducts.map((prod) => ({
    id: prod.id,
    name: prod.name,
    product_uom_qty: prod.qty ?? 0,  // map qty
    price_unit: prod.price ?? 0,     // map price
              ...
              return (
                <View key={item.id || index} style={styles.productRow}>
                  <View style={styles.leftSection}>
                    ...
                    <Text style={styles.productDetail}>
                      Amount: ₹{amount.toFixed(2)} + GST: ₹{gstAmount.toFixed(2)} ({taxNames})
                    </Text>
                  </View>
